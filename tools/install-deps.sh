...
#
# It also logs package installation to /tmp/install.txt.

set -Cue -o pipefail

install_archlinux() {
    pacman --quiet --noconfirm -Sy >>/tmp/install.txt
    pacman --quiet --noconfirm -S \
        ...
        which yamllint >>/tmp/install.txt
}
...
install_debian() {
    ...
    apt-get -q update >>/tmp/install.txt
    ...
    DEBIAN_FRONTEND=noninteractive TZ=UTC apt-get -q install -y \
        ...
        shellcheck libtool pipx yamllint >>/tmp/install.txt
    pipx install uv >>/tmp/install.txt
...
install_debian_codeql() {
    ...
    DEBIAN_FRONTEND=noninteractive TZ=UTC apt-get -q install -y \
        clang cmake libpq-dev make >>/tmp/install.txt
...
install_macos() {
    brew install --quiet \
        ...
        shellcheck uv yamllint libpq >>/tmp/install.txt
}
