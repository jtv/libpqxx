 ...
 */
class pipeline : public internal::transactionfocus
{
public:
  using query_id = long;
  ...
  PQXX_LIBEXPORT explicit pipeline(
	transaction_base &,
	const std::string &Name=std::string{});

  PQXX_LIBEXPORT ~pipeline() noexcept;
   ...
   */
  PQXX_LIBEXPORT void cancel();

  /// Is result for given query available?
  PQXX_LIBEXPORT bool is_finished(query_id) const;
  ...
  PQXX_LIBEXPORT std::pair<query_id, result> retrieve();
   ...
   */
  PQXX_LIBEXPORT int retain(int retain_max=2);


  /// Resume retained query emission (harmless when not needed)
  PQXX_LIBEXPORT void resume();
...
private:
  ...
  PQXX_LIBEXPORT void attach();
  PQXX_LIBEXPORT void detach();
  ...
  PQXX_LIBEXPORT std::pair<pipeline::query_id, result>
