...
template<> struct string_traits<std::chrono::year>
{
  ...
  size_buffer(std::chrono::year const &value) noexcept
  {
    ...
    return string_traits<int>::size_buffer(value);
  }
};
...
template<> struct string_traits<std::chrono::month>
{
  ...
  static char *into_buf(char *begin, char *end, std::chrono::month const &value)
  {
    if ((end - begin) < 3)
      throw conversion_overrun{"Not enough buffer space for month."};
    if (not value.ok()) throw conversion_error{"Month value out of range."};
  ...
  [[nodiscard]] static std::chrono::month from_string(std::string_view text)
  {
    ...
    if (not m.ok()) throw conversion_error{make_parse_error(text)};
    return m;
  }

  [[nodiscard]] static std::size_t
  size_buffer(std::chrono::month const &value) noexcept { return 3u; }
...
template<> struct string_traits<std::chrono::day>
{
  ...
  static char *into_buf(char *begin, char *end, std::chrono::day const &value)
  {
    ...
    if (not value.ok()) throw conversion_error{"Day value out of range."};
    unsigned const day{value};
    begin[0] = number_to_digit(static_cast<int>(day / 10));
    begin[1] = number_to_digit(static_cast<int>(day % 10));
    begin[2] = '\0';
    return begin + 3;
  }
  ...
  [[nodiscard]] static std::chrono::day from_string(std::string_view text)
  {
    ...
    std::chrono::day const d{
      (10 * internal::digit_to_number(text[0]) +
      internal::digit_to_number(text[1])};
    if (not d.ok()) throw conversion_error{make_parse_error(text)};
  }

  [[nodiscard]] static std::size_t
  size_buffer(std::chrono::day const &value) noexcept { return 3u; }
...
template<> struct nullness<std::chrono::year_month_day> :
...
template<> struct string_traits<std::chrono::year_month_day>
{
  ...
  [[nodiscard]] static
  std::chrono::year_month_day from_string(std::string_view text)
  {
    if (std::size(text) < 7)
    ...
    if (not date.ok()) throw conversion_error{make_parse_error(text)};
    return date;
  }
