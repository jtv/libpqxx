...
{
  return std::format(
    "SELECT ('name' || n), n FROM generate_series(1, {})", num_rows);
}
} // namespace
...
int main()
{
  ...
  // Here's the basic way to query the database.  It loads all the result data
  // into our memory, and returns a pqxx::result.
  ...
  // Here's a very basic way to run `process_row` on each of the rows, and add
  // up all the numbers.  It converts each row's two fields to a `std::string`
  ...
  // std::string object, which can involve memory allocations and deallocations
  // internally.
  //
  ...
  // long as r1 (or a copy of it) remains in memory.  Otherwise, the underlying
  ...
  // want to read the fields.  The `for_each()` function inspects your callback
  // to figure out what parameter types it takes, and converts each row's
  // fields to those respective types.
  //
  ...
  // 1. You can start processing the first rows before the query even finishes.
