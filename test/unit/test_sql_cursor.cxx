...
void test_forward_sql_cursor(connection_base &, transaction_base &trans)
{
  ...
  PQXX_CHECK_EQUAL(forward.pos(), 2u, "fetch(0, displacement) affected pos()");

  row = forward.fetch(0);
  PQXX_CHECK_EQUAL(row.size(), 0u, "fetch(0) fetched wrong number of rows");
  PQXX_CHECK_EQUAL(forward.pos(), 2u, "fetch(0) moved cursor");
  PQXX_CHECK_EQUAL(forward.pos(), 2u, "fetch(0) affected pos()");
  ...
  PQXX_CHECK_EQUAL(forward.pos(), 4u, "fetch(1) results in bad pos()");
  ...
  PQXX_CHECK_EQUAL(forward.pos(), 5u, "Not at one-past-end position");
  ...
  PQXX_CHECK_EQUAL(forward.pos(), 5u, "pos() is beyond end");
  ...
  PQXX_CHECK_EQUAL(displacement, 5, "displacement != rows+1");
  PQXX_CHECK_EQUAL(forward3.pos(), 5u, "Bad pos() after fetching all rows");
  ...
  PQXX_CHECK_EQUAL(forward_empty.pos(), 1u, "Bad pos() at end of result");
...
void test_scroll_sql_cursor(connection_base &, transaction_base &trans)
{
  ...
  PQXX_CHECK_EQUAL(scroll.pos(), 0u, "Scroll cursor's initial pos() is wrong");
  ...
  PQXX_CHECK_EQUAL(scroll.pos(), 1u, "Scroll cursor's pos() is broken");
  ...
  PQXX_CHECK_EQUAL(scroll.pos(), 0u, "pos() is not back at zero");
  ...
  PQXX_CHECK_EQUAL(scroll.pos(), 0u, "pos() moved back from zero");
  ...
  PQXX_CHECK_EQUAL(scroll.pos(), 4u, "Position mismatch");
...
void test_adopted_sql_cursor(connection_base &conn, transaction_base &trans)
{
  ...
  PQXX_CHECK_EQUAL(displacement, -4, "Wrong displacement");
  PQXX_CHECK_EQUAL(adopted.pos(), 0u, "Failed to recognize starting position");
  PQXX_CHECK_EQUAL(adopted.endpos(), -1, "endpos() set too early");

  cursor_base::difference_type offset = adopted.move(cursor_base::all());
  PQXX_CHECK_EQUAL(offset, 3, "Unexpected move() offset");
  PQXX_CHECK_EQUAL(adopted.pos(), 4u, "Bad position on adopted cursor");
